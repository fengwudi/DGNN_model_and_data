...
train_val(train_val_data, cawn, args.mode, BATCH_SIZE, NUM_EPOCH, criterion, optimizer, early_stopper, ngh_finders, rand_samplers, logger, GPU)
...
if args.mode == 'i':
    ...
    logger.info('Test statistics: {} new-new nodes -- acc: {}, auc: {}, ap: {}, recall: {}'.format(args.mode, test_new_new_acc, test_new_new_auc,test_new_new_ap, test_new_new_recall ))
    test_new_old_acc, test_new_old_ap, test_new_old_f1, test_new_old_auc, test_new_old_recall = eval_one_epoch('test for {} nodes'.format(args.mode), cawn, test_rand_sampler, test_src_new_old_l, test_dst_new_old_l, test_ts_new_old_l, test_label_new_old_l, test_e_idx_new_old_l)
    logger.info('Test statistics: {} new-old nodes -- acc: {}, auc: {}, ap: {}, recall: {}'.format(args.mode, test_new_old_acc, test_new_old_auc, test_new_old_ap, test_new_old_recall))
