...
def train_val(train_val_data, model, mode, bs, epochs, criterion, optimizer, early_stopper, ngh_finders, rand_samplers, logger, GPU):
    ...
    for epoch in range(epochs):
        ...
        for k in tqdm(range(num_batch), mininterval=5):
            now_time = time.time()
        ...
        memory_use += torch.cuda.max_memory_allocated(device=torch.device('cuda:{}'.format(GPU))) / 1e6
        total_time += epoch_time
        # validation phase use all information
        val_acc, val_ap, val_f1, val_auc, val_recall = eval_one_epoch('val for {} nodes'.format(mode), model, val_rand_sampler, val_src_l,
        ...
        logger.info('average time: {:.2f} s'.format(total_time/(epoch+1)))
        ...
        logger.info('train recall: {:.4f}, val recall: {:.4f}'.format(np.mean(recall), val_recall))
