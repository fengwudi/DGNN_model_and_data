...
def eval_one_epoch(hint, tgan, sampler, src, dst, ts, label):
    val_acc, val_ap, val_f1, val_auc, val_recall = [], [], [], [], []
    with torch.no_grad():
        tgan = tgan.eval()
        TEST_BATCH_SIZE=30
        ...
        for k in range(num_test_batch):
            ...
            val_recall.append(recall_score(true_label, pred_label))
    return np.mean(val_acc), np.mean(val_ap), np.mean(val_f1), np.mean(val_auc), np.mean(val_recall)
...
for epoch in range(NUM_EPOCH):
    ...
    memory_use += torch.cuda.max_memory_allocated(device='cuda:{}'.format(GPU)) / 1e6
    val_acc, val_ap, val_f1, val_auc, val_recall = eval_one_epoch('val for old nodes', tgan, val_rand_sampler, val_src_l, 
    val_dst_l, val_ts_l, val_label_l)

    nn_val_acc, nn_val_ap, nn_val_f1, nn_val_auc, nn_val_recall = eval_one_epoch('val for new nodes', tgan, val_rand_sampler, nn_val_src_l, 
    nn_val_dst_l, nn_val_ts_l, nn_val_label_l)
        
    logger.info('epoch: {}'.format(epoch))
    logger.info('total memory use: {:.2f} MB'.format(memory_use))
    logger.info('train time: {:.2f} s'.format(epoch_time))
    ...
    logger.info('train recall: {:.4f}, val recall: {:.4f}, new node val recall: {:.4f}'.format(np.mean(recall), val_recall, nn_val_recall))
    ...
    if early_stopper.early_stop_check(val_ap):
        ...
        logger.info('early stop total time: {:.2f} s'.format(total_time))
        logger.info('early stop average time: {:.2f} s'.format(total_time/(epoch+1)))
        logger.info('average memory use: {:.2f} MB'.format(memory_use/(epoch+1)))
        tgan.eval()
        break
    else:
...
tgan.ngh_finder = full_ngh_finder
test_acc, test_ap, test_f1, test_auc, test_recall = eval_one_epoch('test for old nodes', tgan, test_rand_sampler, test_src_l, 
test_dst_l, test_ts_l, test_label_l)

nn_test_acc, nn_test_ap, nn_test_f1, nn_test_auc, nn_test_recall = eval_one_epoch('test for new nodes', tgan, nn_test_rand_sampler, nn_test_src_l, 
nn_test_dst_l, nn_test_ts_l, nn_test_label_l)

logger.info('Test statistics: Old nodes -- acc: {:.4f}, auc: {:.4f}, ap: {:.4f}, recall: {:.4f}'.format(test_acc, test_auc, test_ap, test_recall))
logger.info('Test statistics: New nodes -- acc: {:.4f}, auc: {:.4f}, ap: {:.4f}, recall: {:.4f}'.format(nn_test_acc, nn_test_auc, nn_test_ap, nn_test_recall))
logger.info('total time: {:.2f} s'.format(total_time))
logger.info('complete average time: {:.2f} s'.format(total_time/(epoch+1)))
logger.info('average memory use: {:.2f} MB'.format(memory_use/(epoch+1)))
