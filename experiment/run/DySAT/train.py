...
if __name__ == "__main__":
    ...
    if args.time_steps == -1:
        args.time_steps = len(adjs) # use all data
    graphs = graphs[:args.time_steps]
    ...
    # 这里的link prediction是transductive
    ...
    for epoch in range(args.epochs):
        ...
        val_results, test_results, _, _, val_metrics, test_metrics = evaluate_classifier(train_edges_pos,
                                                            train_edges_neg,
                                                            val_edges_pos, 
                                                            val_edges_neg, 
                                                            test_edges_pos,
        ...
        val_aps.append(val_metrics[1])
        val_recalls.append(val_metrics[2])
        val_accs.append(val_metrics[3])
        test_aucs.append(test_metrics[0])
        test_aps.append(test_metrics[1])
        test_recalls.append(test_metrics[2])
        test_accs.append(test_metrics[3])

        memory = torch.cuda.max_memory_allocated() / 1e6
        memory_consume.append(memory)
        elpased_times.append(time.time() - start_time)

        if val_metrics[1] > best_epoch_val:
            best_epoch_val = val_metrics[1]
        ...
        # print(f"len(val_aucs): {len(val_aucs)}")
        print(f"val auc {val_metrics[0]:.4f}, ap {val_metrics[1]:.4f}, recall {val_metrics[2]:.4f}, acc {val_metrics[3]:.4f}")
        print(f"test auc {test_metrics[0]:.4f}, ap {test_metrics[1]:.4f}, recall {test_metrics[2]:.4f}, acc {test_metrics[3]:.4f}")
        print(f"average time {mean(elpased_times):.2f} s, memory {mean(memory_consume):.2f} MB")
    # Test Best Model
