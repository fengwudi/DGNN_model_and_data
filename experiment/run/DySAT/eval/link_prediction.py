...
def evaluate_classifier(train_pos, train_neg, val_pos, val_neg, test_pos, test_neg, source_embeds, target_embeds):
    ...
    test_auc, test_ap, test_recall, test_acc = get_roc_score_t(test_pos, test_neg, source_embeds, target_embeds)
    val_auc, val_ap, val_recall, val_acc = get_roc_score_t(val_pos, val_neg, source_embeds, target_embeds)
    ...
    val_metrics = [val_auc,val_ap,val_recall,val_acc]
    test_metrics = [test_auc,test_ap,test_recall,test_acc]
    return val_results, test_results, val_pred_true, test_pred_true,val_metrics, test_metrics
...
def get_roc_score_t(edges_pos, edges_neg, source_emb, target_emb):
    ...
    pred_labels = pred_all > 0.5
    recall = recall_score(labels_all, pred_labels)
    acc = accuracy_score(labels_all,pred_labels)

    return roc_score, ap, recall, acc
