...
class Logger():
    ...
    def log_minibatch(self, predictions, true_classes, loss, **kwargs):
        ...
        for cl in range(self.num_classes):
            ...
            self.conf_mat_tn[cl]+=conf_mat_per_class.true_negatives[cl]
            for k in self.eval_k_list:
            ...
            if self.set == "TEST":
                ...
                self.conf_mat_tn_list[cl].append(conf_mat_per_class.true_negatives[cl])
    ...
    def log_epoch_done(self):
        ...
        if self.args.target_measure=='AP' or self.args.target_measure=='ap':
        ...
        return eval_measure, epoch_AUC, epoch_AP, epoch_RECALL, epoch_ACC, epoch_F1
    ...
    def get_AUC(self,predictions,true_classes, do_softmax=False):
        if do_softmax:
            probs = torch.softmax(predictions,dim=1)[:,1]
        else:
            probs = predictions
    ...
    def get_RECALL(self,predictions,true_classes, do_softmax=False):
        if do_softmax:
            probs = torch.softmax(predictions,dim=1)[:,1]
        else:
            probs = predictions
        ...
        predictions_np = probs.detach().cpu().numpy()
        true_classes_np = true_classes.detach().cpu().numpy()
        pred_label = predictions_np > 0.5
        
        return recall_score(true_classes_np, pred_label)
    
    def get_ACC(self,predictions,true_classes, do_softmax=False):
        if do_softmax:
            probs = torch.softmax(predictions,dim=1)[:,1]
        else:
            probs = predictions
        ...
        return accuracy_score(true_classes_np, pred_label)
    
    def get_F1(self,predictions,true_classes, do_softmax=False):
        if do_softmax:
            probs = torch.softmax(predictions,dim=1)[:,1]
        else:
            probs = predictions
        ...
        return f1_score(true_classes_np, pred_label)
    ...
    def eval_predicitions(self, predictions, true_classes, num_classes):
        ...
        for cl in range(num_classes):
            ...
            tn = ((true_classes != cl) & (predicted_classes != cl)).sum()
