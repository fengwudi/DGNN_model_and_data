...
def eval(mode='val'):
    ...
    with torch.no_grad():
        ...
        for _, rows in eval_df.groupby(eval_df.index // train_param['batch_size']):
            ...
            pred_pos, pred_neg, t_mem, t_gat = model(mfgs, neg_samples=neg_samples)
...
for e in range(train_param['epoch']):
    ...
    for _, rows in df[:train_edge_end].groupby(group_indexes[random.randint(0, len(group_indexes) - 1)]):
        ...
        optimizer.zero_grad()
        pred_pos, pred_neg, t_mem, t_gat = model(mfgs)
        loss = creterion(pred_pos, torch.ones_like(pred_pos))
        loss += creterion(pred_neg, torch.zeros_like(pred_neg))
        total_loss += float(loss) * train_param['batch_size']
        loss.backward()
        optimizer.step()
        memory_tot += torch.cuda.max_memory_allocated() / 1e6
        t_prep_s = time.time()
        if mailbox is not None:
            eid = rows['Unnamed: 0'].values
            mem_edge_feats = edge_feats[eid] if edge_feats is not None else None
    ...
    total_time += time_tot
    total_memory += memory_tot/cnt
    print('\ttrain loss:{:.4f}  val ap:{:4f}  val auc:{:4f}  val recall:{:.4f}  val acc:{:.4f}'.format(total_loss, ap, auc, recall, acc))
    print('\ttotal time:{:.4f} s sample time:{:.4f} s prep time:{:.4f} s'.format(time_tot, time_sample, time_prep))
    print('\ttotal memory usage:{:.2f} MB'.format(memory_tot/cnt))
...
if mailbox is not None:
    ...
    eval('train')
    eval('val')
ap, auc, recall, acc = eval('test')
print('\tstop at {} epoch:'.format(e))
print('\ttotal average time:{:.2f} s'.format(total_time/(e+1)))
print('\ttotal average memory use:{:.2f} MB'.format(total_memory/(e+1)))
if args.eval_neg_samples > 1:
    print('\ttest AP:{:.4f}  test MRR:{:.4f}  test Recall:{:.4f}  test ACC:{:.4f}'.format(ap, auc, recall, acc))
else:
    print('\ttest AUC:{:.4f}  test AP:{:.4f}  test Recall:{:.4f}  test ACC:{:.4f}'.format(auc, ap, recall, acc))
