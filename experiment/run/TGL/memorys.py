    ...
    def prep_input_mails(self, mfg, use_pinned_buffers=False):
        for i, b in enumerate(mfg):
            if use_pinned_buffers:
                ...
                #b.srcdata['mem'] = self.node_memory.cuda()[b.srcdata['ID'].long()].cuda()
                #b.srcdata['mem_ts'] = self.node_memory_ts.cuda()[b.srcdata['ID'].long()].cuda()
                #b.srcdata['mem_input'] = self.mailbox.cuda()[b.srcdata['ID'].long()].cuda().reshape(b.srcdata['ID'].shape[0], -1)
                #b.srcdata['mail_ts'] = self.mailbox_ts.cuda()[b.srcdata['ID'].long()].cuda()
