...
# print(np.count_nonzero(full_data.labels!=0))
...
for i in range(args.n_runs):
  ...
  for epoch in range(args.n_epoch):
    ...
    val_auc, val_ap, val_f1 = eval_node_classification(tgn, decoder, val_data, full_data.edge_idxs, BATCH_SIZE,
    ...
    logger.info('Epoch {}: train loss: {}, val auc: {:.4f}, val f1: {:.4f}, val ap: {:.4f}, time: {:.2f}'.format(epoch, loss / num_batch, val_auc, val_f1, val_ap, time.time() - start_epoch))
  ...
  if args.use_validation:
    ...
    test_auc, test_ap, test_f1 = eval_node_classification(tgn, decoder, test_data, full_data.edge_idxs, BATCH_SIZE,
                                        n_neighbors=NUM_NEIGHBORS)
  else:
  ...
  logger.info('test auc: {:.4f}, test f1: {:.4f}, test ap: {:.4f}'.format(test_auc, test_f1, test_ap))